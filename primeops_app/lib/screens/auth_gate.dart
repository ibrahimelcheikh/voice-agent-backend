...
import '../data/api_ops_repository.dart';
import '../data/ops_repository.dart';
import '../theme/tokens.dart';
import '../widgets/ui.dart';
import 'shell.dart';

/// Shown only in API mode (USE_MOCK=false). Logs an operator in, hydrates the API
/// repository, then swaps it into [opsRepositoryProvider] and shows the console.
...
class _AuthGateState extends ConsumerState<AuthGate> {
  final _email = TextEditingController(text: 'operator@atlasprimex.ai');
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (user['role'] != 'operator') {
        throw 'This account is not an operator';
      }
      final repo = ApiOpsRepository(api);
      await repo.hydrate();
      ref.read(opsRepositoryProvider.notifier).state = repo;
      setState(() => _ready = true);
    } catch (e) {
      setState(() => _error = 'Login failed — check the backend at $kApiBase. ($e)');
    } finally {
  ...
  Widget build(BuildContext context) {
    if (_ready) return const OpsShell();
    return Scaffold(
                ...
                child: Column(
                  ...
                  children: [
                    Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Image.asset('assets/logo-mark.png', width: 38, height: 38),
                    ...
                    const Text('Operator sign in', textAlign: TextAlign.center, style: TextStyle(color: AppColors.sub)),
